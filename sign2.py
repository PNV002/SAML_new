...
def verify_signature(decoded_saml_response):
    """
    Verify the signature using the extracted SignedInfo, Certificate, and Signature.

    :param decoded_saml_response: The decoded SAML response XML string.
    :type decoded_saml_response: str
    :return: A tuple containing the verification result, certificate, signed info, and signature.
    :rtype: tuple[bool, str, str, str]
    """
    def extract_signed_info(xml_str):
        """
        Extract the SignedInfo from the SAML response XML string.

        :param xml_str: The SAML response XML string.
        :type xml_str: str
        :return: The extracted SignedInfo.
        :rtype: str
        """
    ...
    def extract_certificate(xml_str):
        """
        Extract the X.509 certificate from the SAML response XML string.

        :param xml_str: The SAML response XML string.
        :type xml_str: str
        :return: The extracted X.509 certificate.
        :rtype: str
        """
        root = etree.fromstring(xml_str)
        signature_element = root.find(".//{http://www.w3.org/2000/09/xmldsig#}Signature")
    ...
    def extract_signature(xml_str):
        """
        Extract the SignatureValue from the SAML response XML string.

        :param xml_str: The SAML response XML string.
        :type xml_str: str
        :return: The extracted SignatureValue.
        :rtype: str
        """
        root = etree.fromstring(xml_str)
        signature_element = root.find(".//{http://www.w3.org/2000/09/xmldsig#}Signature")
